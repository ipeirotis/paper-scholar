...
After the inventory and ledger-reuse pass pin the remaining work, follow `references/parallel-retrieval.md`: when subagents are available, launch one worker per distinct source (or per bibliography entry, novelty question, or version baseline as that protocol defines), concurrently up to the platform limit. The coordinator remains the sole ledger writer and validates every worker result before reporting. If subagents are unavailable, use the same work units sequentially and disclose that execution mode.
